...
with
   ...
   connector_split as (
        select
            ...
            trim(replace(split_part(connector_details, ',', 4), 'Status:', '')) as connector_status,
            trim(replace(split_part(connector_details, ',', 5), 'Count:', '')) as connector_count
        from 
            connector_list_exploded
   )
   select * from connector_split
